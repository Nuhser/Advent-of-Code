...
class Solution(aoc.AbstractSolution):
    ...
    def parse(self, puzzle_input: list[str]) -> None:
        ...
        neighbors = get_matching_neighbors(self.pipe_map, self.start_coords, self.is_pointing_to_current_pipe, diagonal=False)
        diff1 = (neighbors[0][0][0] - self.start_coords[0], neighbors[0][0][1] - self.start_coords[1])
        diff2 = (neighbors[1][0][0] - self.start_coords[0], neighbors[1][0][1] - self.start_coords[1])
    ...
    def part1(self) -> tuple[str, (int | float | str | None)]:
        ...
        return f"Loop consists of {main_loop_length} pipes. Furthest distance to start a position {int(main_loop_length / 2)}.", int(main_loop_length / 2)
    ...
    def part2(self) -> tuple[str, (int | float | str | None)]:
        ...
        expanded_pipe_map: dict[tuple[int, int], str] = {}

        max_x = max([key[0] for key in self.pipe_map.keys()])
        max_y = max([key[1] for key in self.pipe_map.keys()])
            ...
            for y in range(-1, (max_y * 2) + 2):
                ...
                for x in range(-1, (max_x * 2) + 2):
                    if ((x, y) in outside_area):
                        ...
                        line.append(Color.YELLOW + expanded_pipe_map[x, y] + Color.DEFAULT)
                    else:
                        line.append(Color.GREEN + expanded_pipe_map[x, y] + Color.DEFAULT)
        ...
        return f"There are {len(inside_loop)} tiles enclosed inside the main loop.", len(inside_loop)
    ...
    def find_main_loop(self) -> list[tuple[int, int]]:
        ...
        while(True):
            if self.verbose:
                print(f"Looking at pipe #{len(main_loop)} '{current_pipe[1]}' (Direction: {current_direction})")
            ...
            match current_pipe[1]:
                ...
                case "7":
                    if (current_direction == (0, -1)):
                        ...
                        new_pipe = (current_pipe[0][0], current_pipe[0][1] + 1)
                case _:
                    raise ValueError(f"ERROR: Unexpected pipe type '{current_pipe[1]}'! You may have lost the main loop.")
